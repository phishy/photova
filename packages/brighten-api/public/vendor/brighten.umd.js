...
`;function nt(){if(typeof document>"u"||document.getElementById("brighten-styles"))return;const b=document.createElement("style");b.id="brighten-styles",b.textContent=at,document.head.appendChild(b)}const u={crop:`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    `}generateFilterPreviews(){const e=this.editor.getLayerManager().getLayers().find(c=>c.type==="image");if(!e||e.type!=="image")return;const i=e.source,r=i instanceof HTMLImageElement?i.src:"canvas";if(this.filterPreviewSource===r&&this.filterPreviewCache.size>0)return;this.filterPreviewSource=r,this.filterPreviewCache.clear();const s=60,a=document.createElement("canvas"),n=i instanceof HTMLImageElement?i.naturalWidth:i.width,o=i instanceof HTMLImageElement?i.naturalHeight:i.height,h=Math.min(s/n,s/o);a.width=Math.round(n*h),a.height=Math.round(o*h);const d=a.getContext("2d");d.drawImage(i,0,0,a.width,a.height);const l=d.getImageData(0,0,a.width,a.height);this.filterPreviewCache.set("none",a.toDataURL("image/jpeg",.7));const g=this.filterEngine.getPresets();for(const c of g){const p=new ImageData(new Uint8ClampedArray(l.data),l.width,l.height),L=this.filterEngine.applyPreset(p,c.id);d.putImageData(L,0,0),this.filterPreviewCache.set(c.id,a.toDataURL("image/jpeg",.7))}}renderAdjustPanel(){return`
      <div class="brighten-panel-header">
        ...
        <div style="margin-bottom: 16px;">
          ...
          <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px;">
            <button class="brighten-btn" data-action="rotate-ccw" style="display: flex; align-items: center; justify-content: center; gap: 6px; padding: 10px;">
              <span style="display: inline-block; width: 20px; height: 20px; transform: scaleX(-1);">${u.rotate}</span>
              90° Left
            </button>
            <button class="brighten-btn" data-action="rotate-cw" style="display: flex; align-items: center; justify-content: center; gap: 6px; padding: 10px;">
              <span style="display: inline-block; width: 20px; height: 20px;">${u.rotate}</span>
              90° Right
            </button>
          </div>
        </div>
        <div style="margin-bottom: 16px;">
          ...
          <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px;">
            <button class="brighten-btn" data-action="flip-h" style="display: flex; align-items: center; justify-content: center; gap: 6px; padding: 10px;">
              <span style="display: inline-block; width: 20px; height: 20px;">${u.flipH}</span>
              Horizontal
            </button>
            <button class="brighten-btn" data-action="flip-v" style="display: flex; align-items: center; justify-content: center; gap: 6px; padding: 10px;">
              <span style="display: inline-block; width: 20px; height: 20px;">${u.flipV}</span>
              Vertical
            </button>
          </div>
        </div>
    ...
    `).join("")+`
      <div data-crop-handle="move" style="
        ...
        cursor: move;
      "></div>
    `}setupCropHandlers(t){this.cropMouseMoveHandler||(this.cropMouseMoveHandler=e=>{var h,d,l,g;const i=this.cropDragState;if(!i.active||!i.startRect||!this.cropRect)return;const r=this.editor.getZoom(),s=(e.clientX-i.startX)/r,a=(e.clientY-i.startY)/r,n=this.editor.getCanvasSize(),o=20;if(i.handle==="move")this.cropRect.x=Math.max(0,Math.min(i.startRect.x+s,n.width-this.cropRect.width)),this.cropRect.y=Math.max(0,Math.min(i.startRect.y+a,n.height-this.cropRect.height));else{if((h=i.handle)!=null&&h.includes("w")){const c=i.startRect.x+s,p=i.startRect.width-s;p>=o&&c>=0&&(this.cropRect.x=c,this.cropRect.width=p)}if((d=i.handle)!=null&&d.includes("e")){const c=i.startRect.width+s;c>=o&&i.startRect.x+c<=n.width&&(this.cropRect.width=c)}if((l=i.handle)!=null&&l.includes("n")){const c=i.startRect.y+a,p=i.startRect.height-a;p>=o&&c>=0&&(this.cropRect.y=c,this.cropRect.height=p)}if((g=i.handle)!=null&&g.includes("s")){const c=i.startRect.height+a;c>=o&&i.startRect.y+c<=n.height&&(this.cropRect.height=c)}}this.updateCropOverlayPosition()},document.addEventListener("mousemove",this.cropMouseMoveHandler)),this.cropMouseUpHandler||(this.cropMouseUpHandler=()=>{this.cropDragState.active=!1,this.cropDragState.handle=null,this.cropDragState.startRect=null},document.addEventListener("mouseup",this.cropMouseUpHandler)),t.addEventListener("mousedown",e=>{const r=e.target.dataset.cropHandle;!r||!this.cropRect||(this.cropDragState.active=!0,this.cropDragState.handle=r,this.cropDragState.startX=e.clientX,this.cropDragState.startY=e.clientY,this.cropDragState.startRect={...this.cropRect},e.preventDefault())})}updateCropOverlayPosition(){if(!this.cropOverlay||!this.cropRect)return;const t=this.editor.getZoom(),e=this.editor.getPan(),i=(g,c)=>({x:g*t+e.x,y:c*t+e.y}),r=i(this.cropRect.x,this.cropRect.y),s=i(this.cropRect.x+this.cropRect.width,this.cropRect.y+this.cropRect.height),a=s.x-r.x,n=s.y-r.y,o=10,h=this.cropOverlay.querySelector("svg");h&&h.querySelectorAll("rect").forEach((c,p)=>{p!==0&&(c.setAttribute("x",String(r.x)),c.setAttribute("y",String(r.y)),c.setAttribute("width",String(a)),c.setAttribute("height",String(n)))});const d=this.cropOverlay.querySelectorAll("[data-crop-handle]"),l={nw:{x:r.x-o/2,y:r.y-o/2},n:{x:r.x+a/2-o/2,y:r.y-o/2},ne:{x:r.x+a-o/2,y:r.y-o/2},e:{x:r.x+a-o/2,y:r.y+n/2-o/2},se:{x:r.x+a-o/2,y:r.y+n-o/2},s:{x:r.x+a/2-o/2,y:r.y+n-o/2},sw:{x:r.x-o/2,y:r.y+n-o/2},w:{x:r.x-o/2,y:r.y+n/2-o/2},move:{x:r.x,y:r.y}};d.forEach(g=>{const c=g.dataset.cropHandle;c&&l[c]&&(g.style.left=`${l[c].x}px`,g.style.top=`${l[c].y}px`,c==="move"&&(g.style.width=`${a}px`,g.style.height=`${n}px`))})}removeCropOverlay(){this.cropMouseMoveHandler&&(document.removeEventListener("mousemove",this.cropMouseMoveHandler),this.cropMouseMoveHandler=null),this.cropMouseUpHandler&&(document.removeEventListener("mouseup",this.cropMouseUpHandler),this.cropMouseUpHandler=null),this.cropDragState={active:!1,handle:null,startX:0,startY:0,startRect:null},this.cropOverlay&&(this.cropOverlay.remove(),this.cropOverlay=null)}setCropRatio(t){if(this.cropRect){if(t){const e=this.cropRect.width*this.cropRect.height,i=Math.sqrt(e*t),r=i/t,s=this.cropRect.x+this.cropRect.width/2,a=this.cropRect.y+this.cropRect.height/2;this.cropRect.width=i,this.cropRect.height=r,this.cropRect.x=s-i/2,this.cropRect.y=a-r/2}this.renderCropOverlay()}}applyCrop(){if(!this.cropRect)return;const e=this.editor.getLayerManager().getLayers().find(a=>a.type==="image");if(!e||e.type!=="image")return;const i=e.source;i instanceof HTMLImageElement?i.naturalWidth:i.width,i instanceof HTMLImageElement?i.naturalHeight:i.height;const r=document.createElement("canvas");r.width=Math.round(this.cropRect.width),r.height=Math.round(this.cropRect.height),r.getContext("2d").drawImage(i,this.cropRect.x,this.cropRect.y,this.cropRect.width,this.cropRect.height,0,0,this.cropRect.width,this.cropRect.height),this.editor.getLayerManager().updateLayer(e.id,{source:r}),this.editor.getCanvasManager().setCanvasSize({width:Math.round(this.cropRect.width),height:Math.round(this.cropRect.height)}),this.editor.saveToHistory("Crop"),this.removeCropOverlay(),this.cropRect=null,this.originalImageData=null,this.showPanel(null),this.setTool("select")}cancelCrop(){this.removeCropOverlay(),this.cropRect=null,this.showPanel(null),this.setTool("select")}rotateImage(t){const i=this.editor.getLayerManager().getLayers().find(d=>d.type==="image");if(!i||i.type!=="image")return;const r=i.source,s=r instanceof HTMLImageElement?r.naturalWidth:r.width,a=r instanceof HTMLImageElement?r.naturalHeight:r.height,n=document.createElement("canvas"),o=Math.abs(t)===90||Math.abs(t)===270;n.width=o?a:s,n.height=o?s:a;const h=n.getContext("2d");h.translate(n.width/2,n.height/2),h.rotate(t*Math.PI/180),h.drawImage(r,-s/2,-a/2),this.editor.getLayerManager().updateLayer(i.id,{source:n}),this.editor.getCanvasManager().setCanvasSize({width:n.width,height:n.height}),this.editor.saveToHistory(`Rotate ${t}°`),this.originalImageData=null}flipImage(t){const i=this.editor.getLayerManager().getLayers().find(h=>h.type==="image");if(!i||i.type!=="image")return;const r=i.source,s=r instanceof HTMLImageElement?r.naturalWidth:r.width,a=r instanceof HTMLImageElement?r.naturalHeight:r.height,n=document.createElement("canvas");n.width=s,n.height=a;const o=n.getContext("2d");t==="horizontal"?(o.translate(s,0),o.scale(-1,1)):(o.translate(0,a),o.scale(1,-1)),o.drawImage(r,0,0),this.editor.getLayerManager().updateLayer(i.id,{source:n}),this.editor.saveToHistory(`Flip ${t}`),this.originalImageData=null}setAiProcessing(t){const e=this.root.querySelector(".brighten-canvas-container");e&&(t&&!this.aiGlowElement?(this.aiGlowElement=document.createElement("div"),this.aiGlowElement.className="brighten-ai-border",e.appendChild(this.aiGlowElement)):!t&&this.aiGlowElement&&(this.aiGlowElement.remove(),this.aiGlowElement=null))}getApiHeaders(){const t={"Content-Type":"application/json"};return this.config.apiKey&&(t.Authorization=`Bearer ${this.config.apiKey}`),t}async removeBackground(){if(!this.config.apiEndpoint){console.error("API endpoint not configured");return}const e=this.editor.getLayerManager().getLayers().find(h=>h.type==="image");if(!e||e.type!=="image")return;const i=e.source,r=document.createElement("canvas");r.width=i instanceof HTMLImageElement?i.naturalWidth:i.width,r.height=i instanceof HTMLImageElement?i.naturalHeight:i.height,r.getContext("2d").drawImage(i,0,0);const a=r.toDataURL("image/png"),n=this.root.querySelector('[data-action="remove-background"]'),o=()=>{this.setAiProcessing(!1),n&&(n.disabled=!1,n.innerHTML=`${u.magic} Remove Background`)};this.setAiProcessing(!0),n&&(n.disabled=!0,n.innerHTML=`${u.magic} Processing...`);try{const h=await fetch(`${this.config.apiEndpoint}/api/v1/background-remove`,{method:"POST",headers:this.getApiHeaders(),body:JSON.stringify({image:a})});if(!h.ok){const g=await h.json();throw new Error(g.error||"Failed to remove background")}const d=await h.json(),l=new Image;l.onload=()=>{this.editor.getLayerManager().updateLayer(e.id,{source:l}),this.editor.saveToHistory("Remove background"),this.originalImageData=null,o()},l.onerror=()=>{throw new Error("Failed to load processed image")},l.src=d.image}catch(h){console.error("Background removal failed:",h),o(),alert(h instanceof Error?h.message:"Background removal failed")}}async unblur(){if(!this.config.apiEndpoint){console.error("API endpoint not configured");return}const e=this.editor.getLayerManager().getLayers().find(h=>h.type==="image");if(!e||e.type!=="image")return;const i=e.source,r=document.createElement("canvas");r.width=i instanceof HTMLImageElement?i.naturalWidth:i.width,r.height=i instanceof HTMLImageElement?i.naturalHeight:i.height,r.getContext("2d").drawImage(i,0,0);const a=r.toDataURL("image/png"),n=this.root.querySelector('[data-action="unblur"]'),o=()=>{this.setAiProcessing(!1),n&&(n.disabled=!1,n.innerHTML=`${u.focus} Unblur / Enhance`)};this.setAiProcessing(!0),n&&(n.disabled=!0,n.innerHTML=`${u.focus} Processing...`);try{const h=await fetch(`${this.config.apiEndpoint}/api/v1/unblur`,{method:"POST",headers:this.getApiHeaders(),body:JSON.stringify({image:a})});if(!h.ok){const p=await h.json();throw new Error(p.error||"Failed to unblur image")}const d=await h.json(),l=r.width,g=r.height,c=new Image;c.onload=()=>{const p=document.createElement("canvas");p.width=l,p.height=g,p.getContext("2d").drawImage(c,0,0,l,g),this.editor.getLayerManager().updateLayer(e.id,{source:p}),this.editor.saveToHistory("Unblur image"),this.originalImageData=null,o()},c.onerror=()=>{o(),alert("Failed to load processed image")},c.src=d.image}catch(h){console.error("Unblur failed:",h),o(),alert(h instanceof Error?h.message:"Unblur failed")}}async upscale(){if(!this.config.apiEndpoint){console.error("API endpoint not configured");return}const e=this.editor.getLayerManager().getLayers().find(h=>h.type==="image");if(!e||e.type!=="image")return;const i=e.source,r=document.createElement("canvas");r.width=i instanceof HTMLImageElement?i.naturalWidth:i.width,r.height=i instanceof HTMLImageElement?i.naturalHeight:i.height,r.getContext("2d").drawImage(i,0,0);const a=r.toDataURL("image/png"),n=this.root.querySelector('[data-action="upscale"]'),o=()=>{this.setAiProcessing(!1),n&&(n.disabled=!1,n.innerHTML=`${u.expand} Upscale 4x`)};this.setAiProcessing(!0),n&&(n.disabled=!0,n.innerHTML=`${u.expand} Processing...`);try{const h=await fetch(`${this.config.apiEndpoint}/api/v1/upscale`,{method:"POST",headers:this.getApiHeaders(),body:JSON.stringify({image:a})});if(!h.ok){const g=await h.json();throw new Error(g.error||"Failed to upscale image")}const d=await h.json(),l=new Image;l.onload=()=>{this.editor.getCanvasManager().setCanvasSize({width:l.naturalWidth,height:l.naturalHeight}),this.editor.getLayerManager().updateLayer(e.id,{source:l}),this.editor.saveToHistory("Upscale image 4x"),this.originalImageData=null,o()},l.onerror=()=>{o(),alert("Failed to load processed image")},l.src=d.image}catch(h){console.error("Upscale failed:",h),o(),alert(h instanceof Error?h.message:"Upscale failed")}}async colorize(){if(!this.config.apiEndpoint){console.error("API endpoint not configured");return}const e=this.editor.getLayerManager().getLayers().find(h=>h.type==="image");if(!e||e.type!=="image")return;const i=e.source,r=document.createElement("canvas");r.width=i instanceof HTMLImageElement?i.naturalWidth:i.width,r.height=i instanceof HTMLImageElement?i.naturalHeight:i.height,r.getContext("2d").drawImage(i,0,0);const a=r.toDataURL("image/png"),n=this.root.querySelector('[data-action="colorize"]'),o=()=>{this.setAiProcessing(!1),n&&(n.disabled=!1,n.innerHTML=`${u.palette} Colorize`)};this.setAiProcessing(!0),n&&(n.disabled=!0,n.innerHTML=`${u.palette} Processing...`);try{const h=await fetch(`${this.config.apiEndpoint}/api/v1/colorize`,{method:"POST",headers:this.getApiHeaders(),body:JSON.stringify({image:a})});if(!h.ok){const p=await h.json();throw new Error(p.error||"Failed to colorize image")}const d=await h.json(),l=r.width,g=r.height,c=new Image;c.onload=()=>{const p=document.createElement("canvas");p.width=l,p.height=g,p.getContext("2d").drawImage(c,0,0,l,g),this.editor.getLayerManager().updateLayer(e.id,{source:p}),this.editor.saveToHistory("Colorize image"),this.originalImageData=null,o()},c.onerror=()=>{o(),alert("Failed to load colorized image")},c.src=d.image}catch(h){console.error("Colorize failed:",h),o(),alert(h instanceof Error?h.message:"Colorize failed")}}async restore(){if(!this.config.apiEndpoint)return;const e=this.editor.getLayerManager().getLayers().find(s=>s.type==="image");if(!e)return;const i=this.root.querySelector('[data-action="restore"]'),r=()=>{this.setAiProcessing(!1),i&&(i.disabled=!1,i.innerHTML=`${u.magic} Restore Photo`)};this.setAiProcessing(!0),i&&(i.disabled=!0,i.innerHTML=`${u.magic} Processing...`);try{const s=e.source,a=document.createElement("canvas"),n=s instanceof HTMLImageElement?s.naturalWidth:s.width,o=s instanceof HTMLImageElement?s.naturalHeight:s.height;a.width=n,a.height=o,a.getContext("2d").drawImage(s,0,0);const d=a.toDataURL("image/jpeg",.95),l=await fetch(`${this.config.apiEndpoint}/api/v1/restore`,{method:"POST",headers:this.getApiHeaders(),body:JSON.stringify({image:d})}),g=await l.json();if(!l.ok)throw new Error(g.error||"Failed to restore image");const c=new Image;c.onload=()=>{const p=document.createElement("canvas");p.width=n,p.height=o,p.getContext("2d").drawImage(c,0,0,n,o),this.editor.getLayerManager().updateLayer(e.id,{source:p}),this.editor.saveToHistory("Restore image"),this.originalImageData=null,r()},c.onerror=()=>{r(),alert("Failed to load restored image")},c.src=g.image}catch(s){console.error("Restore failed:",s),r(),alert(s instanceof Error?s.message:"Restore failed")}}startInpaintMode(){const e=this.editor.getLayerManager().getLayers().find(p=>p.type==="image");if(!e||e.type!=="image")return;this.inpaintMode=!0;const i=e.source,r=i instanceof HTMLImageElement?i.naturalWidth:i.width,s=i instanceof HTMLImageElement?i.naturalHeight:i.height;this.maskCanvas=document.createElement("canvas"),this.maskCanvas.width=r,this.maskCanvas.height=s,this.maskCtx=this.maskCanvas.getContext("2d"),this.maskCtx.fillStyle="black",this.maskCtx.fillRect(0,0,r,s);const a=this.root.querySelector(".brighten-canvas-container");if(!a)return;const n=this.editor.getZoom(),o=this.editor.getPan(),h=r*n,d=s*n;this.maskOverlay=document.createElement("div"),this.maskOverlay.className="brighten-mask-overlay",this.maskOverlay.style.cssText=`position: absolute; left: ${o.x}px; top: ${o.y}px; width: ${h}px; height: ${d}px; cursor: none; z-index: 100;`;const l=document.createElement("canvas");l.style.cssText="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none;";const g=document.createElement("canvas");g.style.cssText="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none;";const c=document.createElement("canvas");c.style.cssText="position: absolute; pointer-events: none; display: none;",this.maskOverlay.appendChild(l),this.maskOverlay.appendChild(g),this.maskOverlay.appendChild(c),a.appendChild(this.maskOverlay),this.setupInpaintDrawing(l,g,c),this.showPanel("ai")}setupInpaintDrawing(t,e,i){if(!this.maskOverlay||!this.maskCanvas||!this.maskCtx)return;const r=[];let s=null,a=!1,n=null;const o=2e3,h={r:200,g:200,b:210},d=(y,m,v)=>{m/=100,v/=100;const w=M=>(M+y/30)%12,k=m*Math.min(v,1-v),x=M=>v-k*Math.max(-1,Math.min(w(M)-3,Math.min(9-w(M),1)));return{r:Math.round(255*x(0)),g:Math.round(255*x(8)),b:Math.round(255*x(4))}},l=()=>{if(!this.inpaintMode)return;const y=this.maskOverlay.getBoundingClientRect();(t.width!==y.width||t.height!==y.height)&&(t.width=y.width,t.height=y.height,e.width=y.width,e.height=y.height);const m=t.getContext("2d"),v=e.getContext("2d");m.clearRect(0,0,t.width,t.height),v.clearRect(0,0,e.width,e.height);const w=Date.now(),k=t.width/this.maskCanvas.width,x=t.height/this.maskCanvas.height;if(r.length>0){m.save(),m.beginPath();for(const C of r)m.moveTo(C.x*k+C.radius*k,C.y*x),m.arc(C.x*k,C.y*x,C.radius*k,0,Math.PI*2);m.clip();const M=r.reduce((C,P)=>C+(w-P.timestamp),0)/r.length,z=Math.min(M/o,1),D=z*300%360,$=90-z*50,T=60+z*25;let E,H,O;if(z>=1)E=h.r,H=h.g,O=h.b;else{const C=d(D,$,T),P=Math.pow(z,2);E=Math.round(C.r*(1-P)+h.r*P),H=Math.round(C.g*(1-P)+h.g*P),O=Math.round(C.b*(1-P)+h.b*P)}m.fillStyle=`rgba(${E}, ${H}, ${O}, 0.4)`,m.fillRect(0,0,t.width,t.height),m.restore()}if(!a&&r.length>0){n===null&&(n=w);const M=w-n,z=M*.1%360,D=.12+Math.sin(M*.005)*.08,$=this.maskCtx.getImageData(0,0,this.maskCanvas.width,this.maskCanvas.height),T=v.createImageData(e.width,e.height);for(let E=0;E<e.height;E++)for(let H=0;H<e.width;H++){const O=Math.floor(H/k),P=(Math.floor(E/x)*this.maskCanvas.width+O)*4;if($.data[P]>128){const ht=(z+H*.5+E*.3)%360,q=d(ht,70,85),j=(E*e.width+H)*4;T.data[j]=q.r,T.data[j+1]=q.g,T.data[j+2]=q.b,T.data[j+3]=Math.round(D*255)}}v.putImageData(T,0,0)}else n=null;s=requestAnimationFrame(l)};l();const g=y=>{const m=this.maskOverlay.getBoundingClientRect(),v=this.maskCanvas.width/m.width,w=this.maskCanvas.height/m.height,k=(y.clientX-m.left)*v,x=(y.clientY-m.top)*w;return{x:k,y:x,scaleX:v,scaleY:w}},c=(y,m,v,w,k)=>{const x=Math.sqrt((v-y)**2+(w-m)**2),M=Math.max(1,Math.floor(x/3)),z=Date.now(),D=this.inpaintBrushSize*k;for(let $=0;$<=M;$++){const T=$/M;r.push({x:y+(v-y)*T,y:m+(w-m)*T,radius:D/2,timestamp:z+$*5})}this.maskCtx.strokeStyle="white",this.maskCtx.lineWidth=D,this.maskCtx.lineCap="round",this.maskCtx.lineJoin="round",this.maskCtx.beginPath(),this.maskCtx.moveTo(y,m),this.maskCtx.lineTo(v,w),this.maskCtx.stroke()},p=(y,m,v)=>{const w=this.inpaintBrushSize*v;r.push({x:y,y:m,radius:w/2,timestamp:Date.now()}),this.maskCtx.fillStyle="white",this.maskCtx.beginPath(),this.maskCtx.arc(y,m,w/2,0,Math.PI*2),this.maskCtx.fill()},L=y=>{if(!this.inpaintDrawState.drawing||!this.maskCtx)return;const{x:m,y:v,scaleX:w}=g(y);c(this.inpaintDrawState.lastX,this.inpaintDrawState.lastY,m,v,w),this.inpaintDrawState.lastX=m,this.inpaintDrawState.lastY=v};this.maskOverlay.addEventListener("mousedown",y=>{const{x:m,y:v,scaleX:w}=g(y);this.inpaintDrawState={drawing:!0,lastX:m,lastY:v},a=!0,p(m,v,w)});const S=y=>{const m=this.maskOverlay.getBoundingClientRect(),v=this.inpaintBrushSize,k=v+4*2;i.width=k,i.height=k,i.style.width=`${k}px`,i.style.height=`${k}px`;const x=i.getContext("2d");x.clearRect(0,0,k,k);const M=k/2,z=v/2;x.beginPath(),x.arc(M,M,z,0,Math.PI*2),x.strokeStyle="rgba(0, 0, 0, 0.7)",x.lineWidth=2,x.stroke(),x.beginPath(),x.arc(M,M,z,0,Math.PI*2),x.strokeStyle="rgba(255, 255, 255, 0.9)",x.lineWidth=1,x.stroke(),i.style.left=`${y.clientX-m.left-M}px`,i.style.top=`${y.clientY-m.top-M}px`,i.style.display="block"};this.maskOverlay.addEventListener("mousemove",y=>{L(y),S(y)}),this.maskOverlay.addEventListener("mouseenter",y=>{S(y)}),this.maskOverlay.addEventListener("mouseup",()=>{this.inpaintDrawState.drawing=!1,a=!1}),this.maskOverlay.addEventListener("mouseleave",()=>{this.inpaintDrawState.drawing=!1,a=!1,i.style.display="none"});const I=()=>{s&&cancelAnimationFrame(s)};this.maskOverlay._cleanup=I}cancelInpaintMode(){if(this.inpaintMode=!1,this.maskCanvas=null,this.maskCtx=null,this.maskOverlay){const t=this.maskOverlay._cleanup;t&&t(),this.maskOverlay.remove(),this.maskOverlay=null}this.showPanel("ai")}async applyInpaint(){if(!this.config.apiEndpoint||!this.maskCanvas){this.cancelInpaintMode();return}const e=this.editor.getLayerManager().getLayers().find(g=>g.type==="image");if(!e||e.type!=="image"){this.cancelInpaintMode();return}const i=e.source,r=document.createElement("canvas");r.width=i instanceof HTMLImageElement?i.naturalWidth:i.width,r.height=i instanceof HTMLImageElement?i.naturalHeight:i.height;const s=r.getContext("2d");s.fillStyle="white",s.fillRect(0,0,r.width,r.height),s.drawImage(i,0,0);const a=r.toDataURL("image/jpeg",.95),n=document.createElement("canvas");n.width=r.width,n.height=r.height;const o=n.getContext("2d");o.fillStyle="black",o.fillRect(0,0,n.width,n.height),o.drawImage(this.maskCanvas,0,0,n.width,n.height);const h=n.toDataURL("image/jpeg",1),d=this.root.querySelector('[data-action="apply-inpaint"]'),l=this.root.querySelector('[data-action="cancel-inpaint"]');this.setAiProcessing(!0),d&&(d.disabled=!0,d.textContent="Processing..."),l&&(l.disabled=!0);try{const g=await fetch(`${this.config.apiEndpoint}/api/v1/inpaint`,{method:"POST",headers:this.getApiHeaders(),body:JSON.stringify({image:a,options:{mask:h}})});if(!g.ok){const I=await g.json();throw new Error(I.error||"Failed to remove objects")}const c=await g.json(),p=r.width,L=r.height,S=new Image;S.onload=()=>{const I=document.createElement("canvas");I.width=p,I.height=L,I.getContext("2d").drawImage(S,0,0,p,L),this.editor.getLayerManager().updateLayer(e.id,{source:I}),this.editor.saveToHistory("Remove objects"),this.originalImageData=null,this.setAiProcessing(!1),this.cancelInpaintMode()},S.onerror=()=>{this.setAiProcessing(!1),this.cancelInpaintMode(),alert("Failed to load processed image")},S.src=c.image}catch(g){console.error("Inpaint failed:",g),this.setAiProcessing(!1),this.cancelInpaintMode(),alert(g instanceof Error?g.message:"Remove objects failed")}}openFilePicker(){const t=document.createElement("input");t.type="file",t.accept="image/*",t.onchange=async()=>{var e;(e=t.files)!=null&&e[0]&&(await this.editor.loadFromFile(t.files[0]),this.resetAdjustments(),this.originalImageData=null,this.currentPreset=null,this.filterPreviewSource=null,this.filterPreviewCache.clear())},t.click()}async exportImage(){const t=await this.editor.export({format:"png",quality:.92});if(this.config.onExport)this.config.onExport(t);else{const e=URL.createObjectURL(t),i=document.createElement("a");i.href=e,i.download="edited-image.png",i.click(),URL.revokeObjectURL(e)}}async saveImage(){if(!this.config.onSave||this.isSaving)return;const t=this.root.querySelector('[data-action="save"]'),e=t==null?void 0:t.innerHTML;try{this.isSaving=!0,t&&(t.disabled=!0,t.innerHTML=`${u.save} Saving...`);const i=await this.editor.export({format:"png",quality:.92}),r=await this.config.onSave(i);return t&&(t.innerHTML=`${u.check} Saved`,setTimeout(()=>{t.innerHTML=e||`${u.save} Save`},2e3)),r}catch(i){throw console.error("Save failed:",i),t&&(t.innerHTML=`${u.close} Failed`,setTimeout(()=>{t.innerHTML=e||`${u.save} Save`},2e3)),i}finally{this.isSaving=!1,t&&(t.disabled=!1)}}addText(){const t=this.editor.getLayerManager(),e=this.editor.getCanvasSize();t.addTextLayer("Double-click to edit",{fontSize:32,color:"#ffffff",transform:{x:e.width/2-100,y:e.height/2-20,scaleX:1,scaleY:1,rotation:0,skewX:0,skewY:0}}),this.editor.saveToHistory("Add text"),this.showPanel("layers")}addLayer(){this.editor.getLayerManager().addDrawingLayer({name:"New Layer"}),this.editor.saveToHistory("Add layer"),this.showPanel("layers")}addShape(t){const e=this.editor.getLayerManager(),i=this.editor.getCanvasSize();e.addShapeLayer(t,{fill:"#3b82f6",transform:{x:i.width/2-50,y:i.height/2-50,scaleX:1,scaleY:1,rotation:0,skewX:0,skewY:0}}),this.editor.saveToHistory(`Add ${t}`),this.showPanel("layers")}toggleLayerVisibility(t){const e=this.editor.getLayerManager().getLayer(t);e&&this.editor.getLayerManager().updateLayer(t,{visible:!e.visible})}async loadImage(t){await this.editor.loadImage(t),this.resetAdjustments(),this.originalImageData=null,this.currentPreset=null,this.filterPreviewSource=null,this.filterPreviewCache.clear()}getEditor(){return this.editor}destroy(){this.keyboardHandler&&document.removeEventListener("keydown",this.keyboardHandler),this.removeCropOverlay(),this.editor.destroy(),this.root.remove()}}function K(b){return new A(b)}function G(b){return new U(b)}const ot={createEditor:K,createEditorUI:G,Editor:A,EditorUI:U};f.AIManager=rt,f.AIProvider=F,f.BaseTool=B,f.BrushTool=V,f.CanvasManager=W,f.CropTool=Q,f.Editor=A,f.EditorUI=U,f.EventEmitter=R,f.FilterEngine=N,f.HistoryManager=Y,f.ImageLoader=_,f.LayerManager=X,f.PluginManager=st,f.RemoveBgProvider=et,f.ReplicateProvider=it,f.TransformTool=tt,f.createEditor=K,f.createEditorUI=G,f.default=ot,Object.defineProperties(f,{__esModule:{value:!0},[Symbol.toStringTag]:{value:"Module"}})});
